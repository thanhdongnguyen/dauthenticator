...
function generateGoogleAuthToken(key) {
  ...
  var aotp = require('aotp');

  return aotp.totp.gen(bin);
}

// Verify a Google Auth Token
function verifyGoogleAuthToken(key, token, timestamp) {
  var bin = decodeGoogleAuthKey(key);
  var aotp = require('aotp');
  ...
  return aotp.totp.verify(token, bin, { window: 1, time: 30, timestamp });
}
